# escreg - App escrow account registry

based on https://github.com/kylebeee/akita-sc/blob/main/projects/akita-sc/smart_contracts/app-registry/contract.algo.ts
...
By default the template creates a single `HelloWorld` contract under escreg folder in the `smart_contracts` directory. To add a new contract:
...
2. Decide what values you want to use for the `allowUpdate` and `allowDelete` parameters specified in [`deploy-config.ts`](./smart_contracts/escreg/deploy-config.ts).
